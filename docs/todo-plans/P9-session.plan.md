...
todos:
  ...
  - id: auto-compact
    content: "新建 subscriptions/auto_compact.ts — 基于 brainBoard.watch('tokens.lastInputTokens') 响应式触发，阈值可配(默认0.6)，超阈值 emit steer 事件请求压缩"
  - id: compact-tool
    content: "新建 tools/compact.ts — 手动触发压缩 + 通过 PathManager 读取 session JSONL（无需 sessionManager 注入）+ claude-code风格结构化摘要"
...
- P4（brainBoard + tokens.* 写入 + model.contextWindow 注册 + watch() hook）
...
### Layer 2: 自动压缩（auto_compact 订阅）

**不再内联在 brain.ts 中**，改为独立订阅 `subscriptions/auto_compact.ts`，解耦策略与框架。

```typescript
// subscriptions/auto_compact.ts
const autoCompact: EventSourceFactory = (ctx) => {
  ...
  return {
    ...
    start(emit) {
      ...
      unwatch = ctx.brainBoard.watch(ctx.brainId, 'tokens.lastInputTokens', (value) => {
        ...
        const utilization = (value as number) / contextWindow;
        if (utilization > threshold) {
          emit({
            type: 'message',
            source: 'subscription:auto_compact',
            content: `[auto_compact] context utilization ${(utilization * 100).toFixed(0)}% > ${(threshold * 100).toFixed(0)}%, requesting compaction`,
            steer: true,
          });
        }
      });
...
**数据获取链路**（全部通过 brainBoard，无额外注入）：
- `tokens.lastInputTokens` ← brain.ts process() 每次 LLM 调用后写入（P4 usage-accumulate）
- `model.contextWindow` ← brain.ts init 时从 ModelSpec 写入（P4 model-init-board）
...
```json
{
  "subscriptions": {
    "enable": ["auto_compact"],
    "config": {
      "auto_compact": { "threshold": 0.6 }
    }
  }
}
```

**压缩执行**：brain.ts 接收到 `source: 'subscription:auto_compact'` 的 steer 事件后，执行压缩逻辑：
- 摘要旧 70% + 保留新 30%
...
compact 工具通过 PathManager 直接读取 session 文件，无需 sessionManager 注入：

```typescript
async execute(args: Record<string, unknown>, ctx: ToolContext): Promise<ToolOutput> {
  const sessionDir = ctx.pathManager.brainDir(ctx.brainId) + '/sessions/';
  const meta = JSON.parse(await readFile(
    ctx.pathManager.brainDir(ctx.brainId) + '/session.json', 'utf-8'
  ));
  const messagesPath = sessionDir + meta.currentSessionId + '/messages.jsonl';
  // 读 JSONL → 压缩 → 写回
  return `{ tokensBefore, tokensAfter, summaryTokens }`;
}
```
...
| 新建 | `tools/compact.ts` |
| 新建 | `subscriptions/auto_compact.ts` |
