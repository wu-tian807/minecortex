---
name: MineCortex框架调研分析
overview: 基于对 agentic_os、agent_fcos、claude-code、cursor-prompts、gemini-cli、manus 六大参考框架的深入调研，分析 MineCortex roadmap 各模块的合理性，梳理实现路径与可参考的提示词/代码引用，并提出概念抽象改进建议。
todos:
  - id: s7-state-cleanup
...
| 框架                 | 核心定位                    | 对 MineCortex 的价值                                                                        |
| ------------------ | ----------------------- | ------------------------------------------------------------------------------------- |
| **agentic_os**     | 文件系统驱动的 Agent OS        | MineCortex 的主对标框架，Agent Loop / MessageBus / 分层 Prompt / 三层压缩 / Spawn Agent 均直接借鉴        |
...
MineCortex roadmap 的设计与业界顶级 Agent 框架高度对齐，核心架构决策（Slot+Tool 两原语、多脑自治、文件即状态）既独特又务实。以下逐模块分析：

### 2.1 合理且紧急的模块

**SS7 删除 state.json + SS8 通用工具补全** -- 正确决策

- agentic_os 的 state 管理分散在 session/memory/task-board 中，MineCortex 将其统一为动态 Slot(工具临时注入) + brain_board(公开动态注册表) 更清晰
...
- agentic_os 的三层压缩(micro-compact / auto-compact / manual-compact)与 MineCortex 设计完全一致
...
- agent_fcos 的 BT 节点最接近(非 LLM 驱动的算法节点)，但 MineCortex 的 ScriptBrain 更统一
...
- MineCortex 的 `priority` + `silent` 双维度控制比 agentic_os 的固定类型更通用
...
1. 一个 `node:fs/watch(ROOT, { recursive: true })` 监听整个 minecortex 工作目录
...
| **Budget**  | token 总量超预算时，按 `priority` 从低到高裁剪          | MineCortex 独创                             |
...
MineCortex 的三大可扩展资源（Subscription / Tool / Slot）采用完全对称的 Loader 模式：
...
> claude-code 大输出保存到磁盘用文件引用。MineCortex 采用组合策略。
...
| **MineCortex** | **摘要旧 70% + 保留新 30%** | **不硬截断，单条 200K→文件引用** | **repairToolPairing** |
...
**不提供 session reset / session new 命令**。agentic_os 的 `switch_session` 在 MineCortex 中**不映射为任何工具**。
...
> MineCortex 采用 observe/plan/act 命名（Minecraft 游戏 agent 语义更贴合），而非直接沿用 agentic_os 的 explore/plan/task。
...
| 维度    | agentic_os                                     | claude-code             | gemini-cli       | cursor                           | **MineCortex**                                    |
| ----- | ---------------------------------------------- | ----------------------- | ---------------- | -------------------------------- | ----------------------------------------------- |
...
> `ToolResult`（string + 图片等富内容）。MineCortex 采用 **JSON Schema 直连 + 多模态返回值**。
...
`brainId#N` 中的 N 是 turn 编号（每次 drain+process 递增），不是 tick——MineCortex 是事件驱动，没有固定频率的 tick。
...
MineCortex 的 Python+WS 方案比以上都更轻量，且完全可扩展——模型可以自行编写任意复杂的 CDP 脚本。
...
- **不采用 Docker**：太重，MineCortex 定位为轻量开发者工具
- **不采用 git worktree**：MineCortex 的 brain 不一定在 git 仓库内操作
...
- **agentic_os 22 个系统指令**：`[references/agentic_os/src/directives/system/](references/agentic_os/src/directives/system/)` — 每个指令独立 .md + .ts（config），按 order 排序，condition 条件加载。MineCortex 简化为两文件
...
来自 gemini-cli 的 Context Efficiency mandate（推荐作为 MineCortex 的全局 directive）：
...
gemini-cli 明确定义了什么任务应该委托、什么应该自己做。建议 MineCortex 在 soul.md 或 directive 中加入：
...
调研后可以更清晰地表达 MineCortex 的核心数据流。所有 Slot 由 `slot-loader` 统一加载各 factory（`soul.ts`、`directives.ts`、`tools.ts`、`skills.ts`、`context-file.ts`、`events.ts`），动态 Slot 由工具运行时注册，无独立 loader：
...
>
> MineCortex 的方案：框架只做 compaction，记忆交给专门的脑区。
