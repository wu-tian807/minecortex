...
| AIRI 层 | MineCortex 对应 | 差异 |
|---------|-------------|------|
| Perception | 感知脑 | AIRI 用 YAML RuleEngine 做信号累积；MineCortex 计划用 Slot+Subscription |
| Reflex | 执行脑（部分） | AIRI 有独立的反射层处理低层即时反应；MineCortex 可借鉴 |
| Conscious | 意识脑 | AIRI 单一 Brain；MineCortex 拆为意识脑+规划脑 |
| Action | Tool 层 | AIRI 是扁平 Action 列表；MineCortex 计划 Primitive→Tool→Skill 三层 |
...
**对 MineCortex 的参考**：MineCortex 计划让 LLM 写 Tool 函数（进化模式），AIRI 的 JS REPL 模式可以作为执行引擎的参考。但 MineCortex 的行为树 tick 执行更适合长时间自主任务。
...
**对 MineCortex 的参考**：这个模式值得借鉴。MineCortex 的 Query Tool 可以设计为类似的链式 DSL，而不是每个查询一个独立的 Tool。这样 LLM 可以在一次调用中组合多个查询条件。
...
**对 MineCortex 的参考**：MineCortex 的执行脑可以借鉴这个反射层，将不需要 LLM 的即时反应（如自动吃食物、看向说话的人、跟随模式）抽离出来，由 Script 代码直接处理，减少 LLM 调用开销。
...
**对 MineCortex 的参考**：MineCortex 的规划脑在执行多步任务时需要类似的上下文管理。可以在 Skill 执行开始时 `enterContext`，结束时 `exitContext` 并写入 Skill 的历史记录。
...
**对 MineCortex 的参考**：MineCortex 的 Tool/Primitive 定义可以参考这个模式：
...
AIRI 共定义 25 个 Action，对标 MineCortex 的 Tool 层：
...
注意：AIRI 的 Action 基本等同于 MineCortex 的 **Tool 层**（组合型原子操作），
...
## 七、对 MineCortex 的启发与借鉴

### 值得借鉴

| 模式 | 说明 | MineCortex 应用场景 |
|------|------|-----------------|
...
### AIRI 的局限性（MineCortex 要避免）

1. **无长期技能积累**：AIRI 没有 Skill 文件，每次都要 LLM 从头编程。MineCortex 的 Skill 进化模式解决了这个问题。
2. **单脑瓶颈**：所有决策都经过一个 Brain，复杂任务时 LLM 负担重。MineCortex 的六脑分工可以更好地分配负载。
3. **无离线规划**：AIRI 没有 HTN/GOAP，无法预先分解复杂任务。MineCortex 的规划脑可以用算法而非 LLM 做任务分解。
4. **JS REPL 依赖强模型**：需要 GPT-4 级别的模型才能稳定输出正确 JS 代码。MineCortex 的 Tool Calling 模式对模型要求更低。
