# Bridge to Minecraft — MineClaw 接入我的世界路径
...
MineClaw 最初通过自研 NeoForge Mod（MineAvatar）接入 Minecraft：
- AgentEntity 继承 PathfinderMob，是一个自定义怪物实体
- MineClaw 通过 TCP/JSON-RPC 向 Mod 发指令
...
- 与 MineClaw 同为 TS/JS 生态，可在**同一进程**中运行，无需 TCP 桥接
...
**目标**：在 MineClaw 中跑通 Mineflayer，验证 Bot 连接、基础行为、physicsTick。

- [ ] 安装 mineflayer 及核心插件到 MineClaw
  ```
...
- [ ] 编写 `src/mc/bot.ts`——Bot 连接与生命周期管理
  ...
  - 与 MineClaw EventBus 对接（MC 事件 → MineClaw Event）
- [ ] 编写 `src/mc/perception.ts`——感知层，分三种模式接入 MineClaw 体系：
...
对应 MineClaw 已有的 Slot（P7）和 Subscription（P8）体系：
...
通过 Mineflayer 事件 → MineClaw EventBus → Event Router → message Slot。
...
这是 MineClaw 与 MindCraft 的核心差异——MindCraft 用固定的 40 个命令，
而我们让 LLM 像 Voyager 一样"写代码"，但写的不是裸 JS，而是 MineClaw Tool 函数。
...
- MineClaw 写 ToolDefinition 文件——有 schema 约束，有权限控制，可热加载，可版本管理
...
可能的方案：
- 方案 A：服务端 Mod 监听自定义频道，MineClaw 通过 Mineflayer 发送 plugin channel 消息
...
## 六、与 MineClaw roadmap 的关系

本文档是 `docs/roadmap.md` 的**领域扩展**——roadmap 定义了 MineClaw 框架本身的演进，
本文档定义了 MineClaw 如何接入 Minecraft 这个具体领域。
