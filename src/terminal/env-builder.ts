 ...
 *   3. bundle/brains/{brainId}/.env（brain 级私有，类 .bashrc，用户编辑）
 *   4. extraEnv（per-call 覆盖）
 *
 * @param brainId       brain 标识符（不含 ':' 后缀）
 * @param pathManager   路径管理器
 * @param useNamespace  是否在 namespace 中运行（true → HOME 重映射到 BRAIN_DIR）
...
export async function buildBrainShellEnv(
  ...
  // 读取 brain 级 .env（用户编辑，存 API key、项目路径等私有变量）
  const brainEnvPath = pathManager.local(brainId).root() + "/.env";
  ...
  const brainDir = pathManager.local(brainId).root();
  ...
  // HOME 重映射：namespace 内将 HOME 指向 brain 私有目录，
  // 使 cd ~、git、pip 等工具的配置文件与宿主机隔离
  if (useNamespace) merged.HOME = brainDir;
  ...
  merged.PATH = [
    `${brainDir}/.local/bin`,   // pipx / pip install --user 命令
    pythonBin,                  // bundle 独立 python3/pip3
    nodeBin,                    // bundle 独立 node/npm/npx
    "/usr/local/bin",           // overlay 后可用的自编译工具
    hostPath,                   // 宿主机基础工具
  ].filter(Boolean).join(":");
